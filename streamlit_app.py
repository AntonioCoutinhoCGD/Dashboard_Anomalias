...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# Upload (versão simples e estável)
# -----------------------------------------------------------------------------
...
def save_cache(df_daily, df_just, just_has_date, df_events, file_hash):
    ...
    with open(STATE_META, "wb") as f:   pickle.dump(meta, f)
    with open(STATE_DAILY, "wb") as f:  pickle.dump(df_daily, f)
    with open(STATE_JUST, "wb") as f:   pickle.dump(df_just, f)
    with open(STATE_EVENTS, "wb") as f: pickle.dump(df_events, f)
...
def load_cache():
    if not os.path.exists(STATE_META): 
        return None
    try:
        with open(STATE_META, "rb") as f:   meta = pickle.load(f)
        with open(STATE_DAILY, "rb") as f:  df_daily = pickle.load(f)
        with open(STATE_JUST, "rb") as f:   df_just = pickle.load(f)
        with open(STATE_EVENTS, "rb") as f: df_events = pickle.load(f)
        return dict(
            meta       = meta,
            df_daily   = df_daily,
            df_just    = df_just,
...
# -----------------------------------------------------------------------------
# -----------------------------------------------------------------------------
# Recomendações ACIONÁVEIS (muito mais específicas)
# -----------------------------------------------------------------------------
...
def _slope7(s: pd.Series, ref_date: pd.Timestamp):
    ...
    # regressão linear simples (sem dependências extra)
...
def _playbook_para_justificacao(just_txt: str):
    ...
    Ex.: 'falta consumíveis' -> abastecer rolos/papel; 'comunicação' -> rede/LAN.
    """
...
def _top_agencias_e_playbook(df_events, end_date, days=30, max_ag=5):
    ...
    # Focar Agências (exclui fornecedores)
    ev = ev[ev["Fonte"] == "Agências"]
    if ev.empty:
        return out
    ...
    for ag, n in by_ag.items():
        ...
        # Principais justificações desta agência
        topj = []
        if "Justificacao" in ev.columns:
            topj = (ev[ev["AgenciaEmpresa"] == ag]
                    .groupby("Justificacao", dropna=False).size()
...
try:
    ...
    for fonte in ["GERAL", "Agências", "Esegur"]:
        for metrica in ["Ruturas", "Indisponiveis", "Anomalias"]:
            ...
            blocos.append({
                "fonte": fonte, "metrica": metrica, "today": today, "m7": m7,
                "wow": prev7, "z": z, "slope7": slope, "abs_d": abs_d, "pct_d": pct_d, "sev": sev
            })

    # 2) Contribuição por Canal no dia de referência (para atacar o maior contributo primeiro)
    contrib = []
    for fonte in ["GERAL", "Agências", "Esegur"]:
        for metrica in ["Ruturas", "Indisponiveis", "Anomalias"]:
    ...
    # Mapear maior contributo por (fonte, metrica)
    top_contrib = {}
    ...
    bullets = []
    # 3.1 – Para cada métrica/fonte com 🟠 ou 🔴, propor ações conforme padrões em justificações
    # Usamos os top 2 drivers de justificação globais (Agências) para orientar ações de semana
    if (df_events is not None) and ("Justificacao" in (df_events.columns if df_events is not None else [])):
        ...
        topj = []
        if not ev_ag.empty:
            topj = (ev_ag.groupby("Justificacao", dropna=False).size()
                          .sort_values(ascending=False).head(3).index.tolist())
        # Ações por cada justificação predominante
        for j in topj:
            acts = _playbook_para_justificacao(j)[:3]
            if acts:
                bullets.append(f"🟠 **Justificação dominante** _{j}_ → " + "; ".join(acts))

    # 3.2 – Roteirização: focar agências reincidentes
    if top_ag_play:
        # Mostrar 3 primeiras para foco semanal
    ...
    # 3.3 – Fornecedor: se 'Esegur' estiver em 🔴/🟠, negociar janelas e frequências
    flag_esegur = any(b["fonte"] == "Esegur" and b["sev"] in ["🔴","🟠"] for b in blocos)
    if flag_esegur:
        # qual metrica/canal domina?
    ...
    bullets = []
    # 4.1 – Tendência (slope7) positiva que sugira deterioração
    trend_warnings = [
        b for b in blocos if (not pd.isna(b["slope7"])) and (b["slope7"] > 0.5) and (b["sev"] in ["🟡","🟠"])
    ]
    ...
    # 4.3 – Reincidência por agência (Pareto 80/20)
    if top_ag_play:
        # percentagem que top 5 representam das ocorrências totais
    ...
    # ---------------------------
    # Extra — Tabela de Prioridades (opcional)
    # ---------------------------
    with st.expander("Tabela de prioridades (score de risco)"):
        pri = []
        for b in blocos:
            score = 0
            if not pd.isna(b["z"]): score += min(3, max(0, int(b["z"])))  # 0..3
            if not pd.isna(b["pct_d"]): score += (2 if b["pct_d"] >= 25 else 1 if b["pct_d"] >= 10 else 0)
            if not pd.isna(b["slope7"]) and b["slope7"] > 0.5: score += 1
